...
st.title("📈 金利を見れば投資はうまくいく")
st.write(
    "米国マクロ経済データの投資環境スコアを自動計算・判定します"
    "。"
)
      ...
      # 画面表示（プロっぽいメトリクスカード）
        ...
        with col2:
          titles = {
              ...
              "summer": "☀️ 【夏】 景気過熱・利上げ開始局面",
              "autumn": "🍂 【秋】 景気減速局面・金利低下局面",
              "late_autumn": "🍁 　晩秋～景気後退の一歩手前",
              "winter": "❄️ 【冬】 景気後退局面・株価急落",
          }
        ...
        advices = {
            ...
            "autumn": {
                "caution": (
                    ...
                    "。株価が高く見えても、レバレッジ局面の罠（株高と社債スプレッド拡大の乖離）に注意が必要です"
                    "。"
                ),
                "action": (
            ...
            "late_autumn": {
                "caution": (
                    ...
                    "、景気後退（冬）が目前に迫っている危険なサインです"
                    "。"
                ),
                "action": (
                    "資産配分見直しの最終局面です。下落に強い商品（公益株や債券など）への入れ替えや、一部を現金化して本格的な下落に備えます"
                    "。"
                ),
            },
            "winter": {
        ...
        df_data = [
            {
                ...
                "判定の理由": "利上げ局面は景気抑制のサイン" if score1 == -2 else "金融緩和・過熱感なく安定局面"
            },
            {
                "項目": "2. 長短金利差", "指標": "10年債 - 政策金利 (%)", "直近値": spread_level, "1年前": "-", 
                "変化 (前年差/比)": "-", "個別スコア": score2,
                "判定の理由": "景気後退の警告信号 (逆イールド)" if score2 == -2 else ("順イールド成長サイクル正常" if score2 == 2 else "中立")
            },
            {
                "項目": "3. 長期金利", "指標": "10年国債利回り (%)", "直近値": dgs10["latest"], "1年前": dgs10["ago"], 
                "変化 (前年差/比)": f"{dgs10_diff:+.2f}", "個別スコア": score3,
                "判定の理由": "金利上昇・経済成長を織り込む上昇" if score3 == 2 else "金利低下・景気減速を先取り"
            },
            {
            ...
            },
            {
                ...
                "判定の理由": "米ドル安・グローバル流動性に寛容" if score5 == 2 else "米ドル高・グローバル資金引き締め圧力"
            },
        ]
        st.dataframe(pd.DataFrame(df_data), use_container_width=True)

        st.markdown("---")
        with st.expander("📖 景気局面（四季）の全体像と解説"):
            st.markdown("""
            参考文書『金利を見れば投資はうまくいく』における、景気サイクルの基本的な考え方です。
            """)
            
            summary_df = pd.DataFrame({
                "季節": ["春", "夏", "秋", "晩秋", "冬"],
                "景況感": ["回復", "過熱", "減速", "冬支度", "後退"],
                "金利データの特徴": ["金利横ばい(様子見)", "利上げ開始", "長期金利低下", "長短金利差急縮小", "利下げ開始"],
            ...
            st.markdown("""
            **投資の心得**
            1. **金利は“売買”ではなく“使う”**: 金利は景気を判断するための強力なツールです。相場の売買に一喜一憂するのではなく、現状のサイクルを客観的に捉えるために使いましょう。
            2. **全ては循環する**: どの局面も永遠には続きません。厳しい局面の後には必ず新しい回復の兆しが訪れます。軸をぶらさず、サイクルの法則に従って冷静に行動することが、大失敗を防ぐ唯一の道です。
            """)
